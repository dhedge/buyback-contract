...
contract EmergencyWithdrawV2 is SetupV2 {
    ...
    function test_ShouldBeAbleToEmergencyWithdraw_PartialAmounts() public {
        ...
        deal(address(pepe), address(L2ComptrollerV2Proxy), 1000e18);

        vm.prank(admin);

        // Withdraw a partial amount of PEPE tokens.
        L2ComptrollerV2Proxy.emergencyWithdraw(address(pepe), 100e18);

        assertEq(
            100e18,
            pepe.balanceOf(admin),
            "Owner balance incorrect"
        );
        assertEq(
            900e18,
            pepe.balanceOf(address(L2ComptrollerV2Proxy)),
            "Comptroller balance incorrect"
        );
    }
    ...
    function test_ShouldBeAbleToEmergencyWithdraw_FullAmount() public {
        ...
        L2ComptrollerV2Proxy.emergencyWithdraw(address(pepe), type(uint256).max);
