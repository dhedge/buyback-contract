...
contract EmergencyWithdraw is Setup {
    ...
    function test_ShouldBeAbleToEmergencyWithdraw_PartialAmounts() public {
        ...
        deal(address(pepe), address(L1ComptrollerProxy), 1000e18);

        vm.prank(admin);

        // Withdraw a partial amount of PEPE tokens.
        L1ComptrollerProxy.emergencyWithdraw(address(pepe), 100e18);

        assertEq(100e18, pepe.balanceOf(admin), "Owner balance incorrect");
        assertEq(900e18, pepe.balanceOf(address(L1ComptrollerProxy)), "Comptroller balance incorrect");
    }
    ...
    function test_ShouldBeAbleToEmergencyWithdraw_FullAmount() public {
        ...
        L1ComptrollerProxy.emergencyWithdraw(address(pepe), type(uint256).max);

        assertEq(1000e18, pepe.balanceOf(admin), "Owner balance incorrect");
        assertEq(0, pepe.balanceOf(address(L1ComptrollerProxy)), "Comptroller balance incorrect");
    }
