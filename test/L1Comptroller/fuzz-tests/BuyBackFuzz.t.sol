...
contract BuyBackFuzz is Setup {
    ...
    function testFuzz_ShouldBurnCorrectAmountOfTokens_WhenReceiverIsSender(uint256 tokenToBurnAmount) public {
        ...
        IERC20Upgradeable(tokenToBurnL1).safeIncreaseAllowance(address(L1ComptrollerProxy), tokenToBurnAmount);
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                ...
                (
                    ...
                    abi.encodeWithSignature("buyBackFromL1(address,address,uint256)", alice, alice, tokenToBurnAmount),
                    1_920_000
                )
            )
        );
        ...
        assertEq(tokenToBurnL1.totalSupply(), tokenSupplyBefore - tokenToBurnAmount, "Wrong total supply");
        assertEq(L1ComptrollerProxy.burntAmountOf(alice), tokenToBurnAmount, "Burnt amount not updated");
    }
